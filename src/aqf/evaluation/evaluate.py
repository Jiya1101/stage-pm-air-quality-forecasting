...
def collect_predictions(model: StagePM, dataset: AQFWindowDataset, device: str, batch_size: int = 32):
    ...
    return preds, stds, targets, regime
...
def evaluate(
    ...
    preds, stds, targets, regime = collect_predictions(model, test_ds, device)
    in_sample = ~held_out_local_mask
    report: dict = {"overall": {}, "per_horizon": {}, "per_bucket": {}, "per_regime": {}, "spatial_holdout": {}}

    report["overall"] = {
        "mae": mae(preds, targets), "rmse": rmse(preds, targets),
        "uncertainty_coverage_95pct": coverage(preds, stds, targets),
    }

    for h_i, h in enumerate(horizons):
        p, t = preds[:, :, h_i], targets[:, :, h_i]
        report["per_horizon"][f"+{h}h"] = {
            "mae": mae(p, t), "rmse": rmse(p, t),
            "csi": critical_success_index(p, t, exceedance_threshold),
        }

    bucket = pollution_bucket(targets)
    report["per_bucket"] = metrics_by_bucket(preds, targets, bucket)

    if regime is not None:
        regime_expanded = np.repeat(regime[:, :, None], preds.shape[-1], axis=-1)
        for r_id, r_name in enumerate(REGIME_CATEGORIES):
            m = regime_expanded == r_id
            if m.sum() == 0:
                continue
            report["per_regime"][r_name] = {"n": int(m.sum()), "mae": mae(preds[m], targets[m]), "rmse": rmse(preds[m], targets[m])}

    report["spatial_holdout"] = {
        "in_sample_stations": {"mae": mae(preds[:, in_sample, :], targets[:, in_sample, :])},
        "held_out_stations": {"mae": mae(preds[:, held_out_local_mask, :], targets[:, held_out_local_mask, :])}
        if held_out_local_mask.any() else None,
    }
